...
>
> 📄 硬件适配（6×A6000 的多卡并行配置与调参方法）见 [`README.md` §6](./README.md)。
...
| 项目 | 论文 | 本机 |
|---|---|---|
| 推理速度标定的 GPU | **NVIDIA A6000** | **6 × NVIDIA RTX A6000（49 GB/卡）** ✅ 型号完全一致 |
| 推理批量 | `diffusion_batch_size=8` | 同（48 GB 放得下，可提到 16–32 换吞吐） |
| 低显存降级 | 未使用 | 未使用（`LOW_MEMORY=False`） |
| CPU / 内存 | 未说明 | 大内存多核 ➜ MPNN 与几何指标多进程并行 |
| 折叠后端硬件要求 | AF3 / Chai-1 通常按 A100 80 GB 量级报 | A6000 48 GB 可跑中小复合物，大批量建议分片铺多卡 |

**结论：Fig. 1d 的绝对秒数可以与本机直接对标**（同一型号的卡）。
本目录的脚本已按这套硬件配置好（每个实验内部自动铺满空闲卡），详见 `README.md §6`。
