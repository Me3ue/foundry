#!/usr/bin/env bash
# One-click replacement-NMF sweep on ZKP-suitable layers.
#
...
#
# Optional:
#   INCLUDE_BASELINE=1  also run a no-NMF baseline
#   INCLUDE_ALL=1       also run the union of all ZKP-suitable layers
#
# Usage:
#   bash models/rfd3/scripts/run_nmf_zkp_single_structure_sweep.sh
# Optional env:
#   DATA=... LOG_ROOT=... CKPT=... PYTHON=python SEED=1 MAX_EPOCHS=50
...
LOG_ROOT="${LOG_ROOT:-/home/zzj/protein/foundry/logs/train_nmf_zkp}"
...
SWEEP_DIR="${SWEEP_DIR:-${LOG_ROOT}/sweep_nmf_zkp_single_structure_${SWEEP_STAMP}}"
...
if [[ "${INCLUDE_BASELINE}" == "1" ]]; then
  JOBS+=("baseline|single_structure|name=baseline ckpt_config.path=${CKPT} ckpt_config.reset_optimizer=true")
fi
JOBS+=(
  "zkp_encoder|nmf_zkp_encoder_single_structure|name=zkp_encoder"
  "zkp_proj|nmf_zkp_proj_single_structure|name=zkp_proj"
  "zkp_head|nmf_zkp_head_single_structure|name=zkp_head"
)
if [[ "${INCLUDE_ALL}" == "1" ]]; then
  JOBS+=("zkp_all|nmf_zkp_all_single_structure|name=zkp_all")
fi
...
tags = []
for p in sorted(sweep_dir.glob("*.exit_code")):
    tags.append(p.stem.replace(".exit_code", "") if False else p.name.replace(".exit_code", ""))
preferred = ["baseline", "zkp_encoder", "zkp_proj", "zkp_head", "zkp_all"]
tags = [t for t in preferred if (sweep_dir / f"{t}.exit_code").exists()] + [
    t for t in tags if t not in preferred
]
...
def load_replacements(tag: str) -> str:
    for p in [
        ...
        sweep_dir / f"nmf-zkp-{tag.replace('zkp_', '')}-single-structure.nmf_replacements.json",
...
rows = []
md = [
    "# ZKP-oriented NMF single-structure sweep",
    "",
...
md.extend(
    [
        ...
        "## Notes",
        "",
        "- Encoder-group NMF is injected into the full model (`apply_to_token_initializer=true`) because the best proof targets live in `TokenInitializer`.",
